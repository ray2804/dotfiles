...
function init_page_layout() {
    ...
    if(document.getElementById("text-wrap")) {
       ...
       mw.util.addPortletLink ('p-displayOptions', 'javascript:toggle_layout();', ws_msg('layout'), 'option-textLayout', '' );
        ...
        layout_num = 0;
        if ($('#dynamic_layout_overrider').size()) {
            var layout_name = $('#dynamic_layout_overrider').text();
            set_layout_by_name(layout_name);
        } else {
...
function refresh_pagenumbers() {
    ...
    $('<span id="dummy-pagenumber" class="pagenum" style="display:none;"></span>').appendTo('div#mw-content-text');
    ...
    for(var i=self.pagenum_ml.length-1; i>=0; i--) {
        ...
        var link_str = "<a href=\""+escapeQuotesHTML(page_url)+"\"" + class_str + " title=\""+escapeQuotesHTML(pagetitle)+"\">"+escapeQuotesHTML(num)+"</a>" 
