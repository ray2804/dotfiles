(defproject local-client "0.0.1"
  ...
  :injections [(require '[clojure.zip :as zip]
                        ...
                        '[clojure.test :refer [deftest testing is are with-test
                                               with-test-out function? successful?
                        ...
                        '[garden.units :as gunits :refer [px pt em]]
                        '[garden.color :as gcolor :refer [hsl]]

                        '[plumbing.graph :as graph]
                        '[plumbing.core :as plumbing :refer [for-map map-vals map-keys
                                                             map-from-keys map-from-vals
                                                             dissoc-in keywordize-map
                                                             sum singleton indexed positions
                                                             distinct-id ?>> ?> fn-> fn->> <-
                                                             memoized-fn swap-pair! millis
                                                             get-and-set! letk fnk defnk]]
